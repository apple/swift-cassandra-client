...
final class Tests: XCTestCase {
  ...
  func testShutdownELGShared() {
    ...
    let cassandraClient = CassandraClient(
      eventLoopGroupProvider: .shared(eventLoopGroup), configuration: configuration)
    let tableName = "test_\(DispatchTime.now().uptimeNanoseconds)"
    XCTAssertNoThrow(
      try cassandraClient.run("create table \(tableName) (id int primary key);").wait())
    XCTAssertNoThrow(try cassandraClient.shutdown())
  ...
  func testQueryIterator() {
    ...
    let paginatedIDs = try! self.cassandraClient.query(
      "select id, data from \(tableName);", pageSize: Int32(1000)
    )
  ...
  func testPagingToken() throws {
    ...
    let initialPages = try self.cassandraClient.query(
      "select id, data from \(tableName);", pageSize: Int32(5)
    ).wait()
    ...
    let offsetPages = try self.cassandraClient.execute(
      statement: statement, pageSize: Int32(5), on: nil
    ).wait()
      ...
      runAsyncAndWaitFor(
        {
          ...
          let paginatedRows = try await self.cassandraClient.query(
            "select id, data from \(tableName);", pageSize: Int32(300))
  ...
  func testSelectIn() throws {
    ...
    XCTAssertEqual(
      Set(rows.compactMap { row in row.column(0)?.int32 }), Set(selectIDs), "result should match")
  }
  ...
  func testDecoding() {
    ...
    for (index, item) in result.enumerated() {
      ...
      XCTAssertEqual(
        item.col8.timeIntervalSince1970, data[index].col8.timeIntervalSince1970, accuracy: 1,
  ...
  func testDataTypes() {
    ...
    for index in Int8(0)..<Int8(10) {
      ...
      XCTAssertEqual(
        row.column("col10")?.date.map { Double($0 * 86400) } ?? 0.0, now, accuracy: 100_000,
        "expected value to match")
      XCTAssertEqual(
        row.column("col11")?.timestamp.map { Double($0 / 1000) } ?? 0.0, now, accuracy: 1,
        "expected value to match")
  ...
  extension XCTestCase {
    ...
    func runAsyncAndWaitFor(
      _ closure: @escaping @Sendable () async throws -> Void, _ timeout: TimeInterval = 3.0
    ) {
