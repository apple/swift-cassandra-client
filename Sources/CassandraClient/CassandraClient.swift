...
public class CassandraClient: CassandraSession {
  ...
  public init(
    eventLoopGroupProvider: EventLoopGroupProvider = .createNew, configuration: Configuration,
    logger: Logger? = nil
  ) {
    ...
    self.defaultSession = Session(
      configuration: self.configuration, logger: self.logger,
  ...
  public func execute(statement: Statement, on eventLoop: EventLoop?, logger: Logger? = .none)
  ...
  public func execute(
    ...
    self.defaultSession.execute(
      statement: statement, pageSize: pageSize, on: eventLoop, logger: logger)
  }
  ...
  public func withSession(
    keyspace: String?, logger: Logger? = .none, handler: (CassandraSession) throws -> Void
  ) rethrows {
  ...
  public func withSession<T>(
    keyspace: String?, logger: Logger? = .none, handler: (CassandraSession) -> EventLoopFuture<T>
  ) -> EventLoopFuture<T> {
    let session = self.makeSession(keyspace: keyspace, logger: logger)
  ...
  extension CassandraClient {
    ...
    /// - Returns: The ``PaginatedRows``.
    @available(macOS 12, iOS 15, tvOS 15, watchOS 8, *)
    public func execute(statement: Statement, pageSize: Int32, logger: Logger? = .none) async throws
      -> PaginatedRows
    {
      try await self.defaultSession.execute(
        statement: statement, pageSize: pageSize, logger: logger)
    }
    ...
    @available(macOS 12, iOS 15, tvOS 15, watchOS 8, *)
    public func withSession(
      keyspace: String?, logger: Logger? = .none, closure: (CassandraSession) async throws -> Void
    ) async throws {
      let session = self.makeSession(keyspace: keyspace, logger: logger)
      defer {
        do {
    ...
    @available(macOS 12, iOS 15, tvOS 15, watchOS 8, *)
    public func withSession<T>(
      keyspace: String?, logger: Logger? = .none, handler: (CassandraSession) async throws -> T
    ) async throws -> T {
      let session = self.makeSession(keyspace: keyspace, logger: logger)
      defer {
        do {
