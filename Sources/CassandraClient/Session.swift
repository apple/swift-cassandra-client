...
public protocol CassandraSession {
  ...
  func execute(statement: CassandraClient.Statement, on eventLoop: EventLoop?, logger: Logger?)
  ...
  func execute(
    statement: CassandraClient.Statement, pageSize: Int32, on eventLoop: EventLoop?, logger: Logger?
    ...
    func execute(statement: CassandraClient.Statement, logger: Logger?) async throws
    ...
    /// - Returns: The resulting ``CassandraClient/PaginatedRows``.
    @available(macOS 12, iOS 15, tvOS 15, watchOS 8, *)
    func execute(statement: CassandraClient.Statement, pageSize: Int32, logger: Logger?)
...
extension CassandraSession {
  ...
  ///
  /// - Returns: The resulting ``CassandraClient/Rows``.
  internal func execute(statement: CassandraClient.Statement, logger: Logger? = .none)
    -> EventLoopFuture<CassandraClient.Rows>
  {
...
extension CassandraSession {
  ...
  public func run(
    ...
    self.query(command, parameters: parameters, options: options, on: eventLoop, logger: logger).map
    { _ in () }
  }
  ...
  public func query(
    ...
    do {
      let statement = try CassandraClient.Statement(
        query: query, parameters: parameters, options: options)
...
extension CassandraClient {
  internal final class Session: CassandraSession {
    ...
    internal init(
      configuration: Configuration, logger: Logger, eventLoopGroupContainer: EventLoopGroupConainer
    ) {
    ...
    func execute(statement: Statement, on eventLoop: EventLoop?, logger: Logger? = .none)
    ...
    func execute(
      statement: Statement, pageSize: Int32, on eventLoop: EventLoop?, logger: Logger? = .none
    ) -> EventLoopFuture<CassandraClient.PaginatedRows> {
      let eventLoop = eventLoop ?? self.eventLoopGroup.next()
...
// MARK: - Cassandra session with async-await support
  ...
  extension CassandraSession {
    ...
    public func query<T>(
      ...
      let rows = try await self.query(
        query, parameters: parameters, options: options, logger: logger)
  ...
  extension CassandraClient.Session {
    @available(macOS 12, iOS 15, tvOS 15, watchOS 8, *)
    func execute(statement: CassandraClient.Statement, logger: Logger? = .none) async throws
      -> CassandraClient.Rows
    {
      let logger = logger ?? self.logger
    ...
    @available(macOS 12, iOS 15, tvOS 15, watchOS 8, *)
    func execute(statement: CassandraClient.Statement, pageSize: Int32, logger: Logger? = .none)
      async throws -> CassandraClient.PaginatedRows
    {
      try statement.setPagingSize(pageSize)
...
private func futureSetCallback(
  _ future: OpaquePointer, completion: @escaping (Result<Void, Error>) -> Void
) {
...
private func futureSetResultCallback(
  _ future: OpaquePointer, completion: @escaping (Result<CassandraClient.Rows, Error>) -> Void
) {
  let closure = unmanagedRetainedClosure {
    DispatchQueue.global().async {
      let resultCode = cass_future_error_code(future)
