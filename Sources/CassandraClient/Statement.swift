      ...
      for (index, parameter) in parameters.enumerated() {
        ...
        case .bool(let value):
          result = cass_statement_bind_bool(
            self.rawPointer, index, value ? cass_bool_t(1) : cass_bool_t(0))
        ...
        case .bytesUnsafe(let buffer):
          result = cass_statement_bind_bytes(
            self.rawPointer, index, buffer.baseAddress, buffer.count)
      ...
      try checkResult {
        pagingStateToken.withUnsafeBytes {
          ...
          return cass_statement_set_paging_state_token(
            self.rawPointer, buffer.baseAddress, buffer.count)
        }
      }
    }
